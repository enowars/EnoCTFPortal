...
class Heart {
  ...
  ventricleDepolarization(time) {
    ...
    if (time <= this.vdDuration / 3) y = (randomGaussian(8, 2) * (this.vdDuration - time)) / 6;
...
class ECG {
  ...
  plotValues() {
    ...
    for (let i = 1; i < this.values.length; i++) {
      ...
      if (i + 5 > this.values.length) {
        circle(this.graphZero.x + this.values[i].x, this.graphZero.y + this.values[i].y, this.values.length / i);
      }
    }
    pop();
  }
  ...
  updateHemoglobin() {
    document.getElementById('hemoglobin-value').innerHTML = randomGaussian(14, 0.1).toFixed(1);
  }
  updateTemperature() {
    document.getElementById('temperature-value').innerHTML = randomGaussian(37, 0.1).toFixed(1);
  }
  updateVentilation() {
    document.getElementById('minute-ventilation-value').innerHTML = randomGaussian(6, 0.5).toFixed(2);
  }
...
// Initialize the ecg
let ecg = new ECG({ x: 0, y: 110 }, [{ x: 0, y: 0 }], width);
